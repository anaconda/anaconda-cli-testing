# This test verfies the `anaconda auth api-key` command and get the API key after a full OAuth login flow.
...
def test_auth_api_key_command(ensureConda,run_cli_command, api_request_context, credentials, urls, request):
    """
    This test verifies that 'anaconda auth api-key' returns a valid API key
    after performing full OAuth login (API + browser + CLI).
    """
    logger.info("Running full login flow before API key check...")

    # Get required fixtures manually
