...
def test_anaconda_token_install_with_oauth(
    ...
    assert condarc_path.exists(), (
        f".condarc file should exist at {condarc_path} after token install with .condarc accepted. "
        f"Process exit code: {token_proc.returncode}, Condarc state: {state['condarc']}, "
        f"HOME: {clean_home}, Files in clean_home: {list(Path(clean_home).iterdir()) if Path(clean_home).exists() else 'N/A'}"
    )
    ...
    # Accept ToS for the org channel and default channels before searching
    # The org channel might be configured as a custom channel, so we accept ToS for repo/main as well
    ...
    # launch_subprocess uses text=True, so communicate() returns strings
    # stderr is merged into stdout, so stderr will be empty string
